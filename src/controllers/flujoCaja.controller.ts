...
export async function getMovimientosCajaUsuarioxConfirmar(req: Request, res: Response): Promise<Response> {
    try {
        ...
        if(body.c_compania) {
            ...
            const conn = await connect();
            const [response, column2] : [any, any] = await conn.query(`CALL sp_Obtener_Movimientos_Por_Confirmar(?,?,?,?,?)`,
            [body.c_compania,body.c_usuariofcu,body.c_agencia,body.c_agenciaotra,body.c_codigousuario]);
            await conn.end();
